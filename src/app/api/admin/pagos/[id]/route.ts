...
export const GET = ruta("pagos GET id", async (_req: Request, ctx: Ctx) => {
  const sinPermiso = await exigirModulo("administracion")
  if (sinPermiso) return sinPermiso
  const { id } = await ctx.params
  return obtenerPago("pago", id)
})
 ...
 */
export const DELETE = ruta("pagos DELETE", async (_req: Request, ctx: Ctx) => {
  const sinPermiso = await exigirModulo("administracion")
  if (sinPermiso) return sinPermiso
  const { id } = await ctx.params
  return anularPago("pago", id)
})

/** Editar un recibo ya cargado, sin perder su id — así los enlaces desde el
 *  extracto del banco siguen funcionando. */
export const PATCH = ruta("pagos PATCH", async (req: Request, ctx: Ctx) => {
  const sinPermiso = await exigirModulo("administracion")
  if (sinPermiso) return sinPermiso
  const { id } = await ctx.params
  return editarPago("pago", req, id)
})
